...
In adherence with PEP 8, ``underscore_case`` is used for variables and class definitions. ``TitleCase`` signifies a class. ``lowerCamelCase`` is rarely used, except where it's the convention, such as for Qt programming. Mixing cases e.g.
